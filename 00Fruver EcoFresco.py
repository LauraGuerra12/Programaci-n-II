        ...
        for producto in self.productos:
            if producto.nombre.lower() == nombre.lower():
                if producto.cantidad >= cantidad_vendida:
                    precio_a_pagar = producto.calcular_precio_total(cantidad_vendida)
                    producto.cantidad -= cantidad_vendida
                    print(f"\nVenta realizada: {cantidad_vendida} {producto.unidad}(s) de {producto.nombre}.")
                    print(f"Precio a pagar: ${precio_a_pagar:.2f}")
                    return
                else:
